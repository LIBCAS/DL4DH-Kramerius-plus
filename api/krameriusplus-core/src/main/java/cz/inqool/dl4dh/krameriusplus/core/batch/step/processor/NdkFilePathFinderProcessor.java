...
import cz.inqool.dl4dh.krameriusplus.core.enricher.ndk.MetsExplorer;
import cz.inqool.dl4dh.krameriusplus.core.enricher.ndk.MetsFileLocatorService;
...
public class NdkFilePathFinderProcessor implements ItemProcessor<Page, NdkFilePathWrappedPage> {
    ...
    public NdkFilePathWrappedPage process(Page item) {
        ...
        Utils.notNull(pageMetsPath, () -> new NdkEnrichmentException("Path to METS file for page: " + item.getId() + " was not extracted from main METS file.",
                NDK_PAGE_DIV_NOT_FOUND));
        Utils.isTrue(Files.exists(pageMetsPath), () -> new NdkEnrichmentException("METS file for page: " + item.getId() + " in path: " + pageMetsPath +
